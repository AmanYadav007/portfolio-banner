...
function exitHandler(e) {
    ...
    console.log("clicked on", e.id), e.id, window.open(window.target_url1)
}
...
function init() {
    gsap.registerPlugin(DrawSVGPlugin), new Border(borderHolder, {
        ...
        .set(document.getElementById("changing"), {
            x: 115,
            y: 50
        })
        .set(document.getElementById("changing2"), {
            x: -250,
            y:55
        })
        .set(document.getElementById("changing3"), {
            x: -250,
            y: 55
        })
        .addLabel("one").add(drawLine1(), "one").to([logo, text1], {
            duration: 1.5,
            stagger: .5,
            autoAlpha: 1,
            drawSVG: "0%"
        }, "one").add(drawLine2(), "one").addLabel("two", "+=1").to([logo,line2holder, line1holder, text1 ], {
            duration: .5,
            autoAlpha: 0
        }, "two").addLabel("twoA", "+=.25").add(drawLine3(), "twoA").to([logo2, text2 ], {
            duration: 1.5,
            stagger: .5,
            autoAlpha: 1
        }, "twoA").addLabel("three", "+=1").to([logo2, line3holder, text2 ], {
                duration: .3,
                autoAlpha: 0
            },
            // "three").addLabel("threeA", "+=.25").add(drawLine5(), "threeA").to([logo, text3, text3a, footnote, "#HandWritten .textReveal", text3b, btn_action], {
            //     duration: 1.5,
            //     stagger: .5,
            //     autoAlpha: 1
            // },
            "three").addLabel("threeA", "+=.25").add(drawLine5(), "threeA").to([logo3,text3, footnote, "#HandWritten .textReveal", text3b, btn_action], {
                duration: .5,
                stagger: .5,
                autoAlpha: 1,
                drawSVG: "100%"
            ...
            // "three").addLabel("threeA", "+=.5").to([line4pathholder, line4path, text3, text3a,btn_action], {
            //     duration: .5,
            //    autoAlpha:0
            //   }, "threeA")
            // 	 .to([logo,footnote], {
